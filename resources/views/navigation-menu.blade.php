<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link href="{{ route('customize.index') }}" :active="request()->routeIs('customize.index')">
                        {{ __('Customize Pizza') }}
                    </x-nav-link>
                    ...
                    @endif
                    <a href="{{ route('cart.index') }}" class="inline-flex items-center px-1 pt-1 border-b-2 {{ request()->routeIs('cart.index') ? 'border-indigo-400 text-gray-900' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }} text-sm font-medium leading-5 focus:outline-none transition duration-150 ease-in-out relative">
                        <!-- Cart Icon using Font Awesome style -->
                        <i class="fas fa-shopping-cart text-xl"></i>
                        @if(Auth::check() && Auth::user()->cart && Auth::user()->cart->items_count > 0)
                        <span class="absolute -top-1 -right-1 text-xs bg-red-600 text-white rounded-full h-5 w-5 flex items-center justify-center font-bold">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link href="{{ route('customize.index') }}" :active="request()->routeIs('customize.index')">
                {{ __('Customize Pizza') }}
            </x-responsive-nav-link>
            ...
            <div class="flex items-center px-4">
                @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                    <div class="shrink-0 me-3">
                        <img class="size-10 rounded-full object-cover" src="{{ Auth::check() ? Auth::user()->profile_photo_url : 'https://ui-avatars.com/api/?name=Guest&color=7F9CF5&background=EBF4FF' }}" alt="{{ Auth::check() ? Auth::user()->name : 'Guest' }}" />
                    </div>
                @endif
